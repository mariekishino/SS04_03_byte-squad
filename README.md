...
ホームの目的説明、チュートリアル、手入力で挑戦するゲーム任務1、送り方を選ぶゲーム任務2、欠けた末尾を補うゲーム任務3、方式情報を含めて送るゲーム任務4まで実装しています。旧ミッション1はチュートリアル、旧ミッション2はゲーム任務1として分離しました。詳細は[ゲーム仕様](docs/02_game_design.md)を参照してください。
...
6. **完了：ゲーム任務4** — 方式情報1 Bを実際に付けて送り、受信側が読み方を決定。本体と送信合計を分けて表示。
...
次の実装対象はRLE章の自由実験と用語カードです。RLE章を一通り完成させた後、`ABCABCABC`の並びの繰り返しを題材に、LZ77の考え方へ進みます。後続章は未実装です。詳細は[学習ロードマップ](docs/05_acceptance_and_roadmap.md#学習ロードマップ合意済み)を参照してください。
...
任務ごとに下書き、方式、再生位置、ヒントの開示段階、クリア状況を保持します。ホーム経由で別の任務に移っても混ざりません。「任務を最初から」「もう一度挑戦」は任務1・2・4では空のRLE入力、任務3では欠損した初期データ`(4,A)`へ戻します。ページを再読み込みすると全任務の状態と進捗をリセットします。
...
1. ホームの「任務4：見えない1バイト」、または任務3のクリア画面から進みます。
2. 元データは`AAAAAABBBBCC`、上限7 B。方式情報1 Bを最初から容量に含めます。本体が空のRLEは送信できません。
3. RLEで`(6,A) (4,B) (2,C)`を作ると、本体6 B＋方式情報1 B＝7 B。実際の送信データは`[1,6,65,4,66,2,67]`です。
4. 地球は先頭の`1`を読んでRLEを選び、その後の本体から12 Bを復元します。「前へ」「次へ」で方式読み取りのステップも観察できます。
5. 無圧縮を選ぶと先頭は`0`、本体12 B＋方式情報1 B＝13 B。「予算を超えて試す」で、内容は一致しても上限を6 B超えることを確認できます。
6. 方式を切り替えてもRLEの組と下書きは保持します。誤答を送って修正することもでき、再挑戦では空のRLE入力へ戻ります。

方式情報は元の文字データに含めません。この1バイトはゲーム独自形式で、ZIPの仕様ではありません。クリア後はRLEを振り返り、`ABCABCABC`の並びの繰り返しを次の学習への問いとして提示します。自由実験・用語カード・後続章は準備中です。
...
| [src/components/GameMission.tsx](src/components/GameMission.tsx) | ゲーム任務1～4の方式選択・編集・復元・結果 |
| [src/game/missions.ts](src/game/missions.ts) | チュートリアルとゲーム任務1～4の教材・予算・依頼・ヒント |
...
| [e2e/mission-four.spec.ts](e2e/mission-four.spec.ts) | 方式情報の表示・読み取り、無圧縮の超過からRLEで成功する操作の検証 |
...
型検査・自動テスト111件（コア77件＋チュートリアル状態3件＋ゲーム状態31件）・ブラウザテスト36件・Webアプリとコアのビルドが成功しました。
資料の境界値・異常系に加え、固定シードで生成した100入力の往復変換とトレースの一致を確認しています。旧5問の数値例、合法だが予算を超える組み方、欠損と修理もコアで検証しています。

- 操作できるのはホーム、チュートリアル、ゲーム任務1～4です。自由実験、用語カードは未実装です。豆知識はRLEの方式名・仕組み・学ぶ理由・歴史と出典を掲載しています。
