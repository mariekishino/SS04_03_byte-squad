...
ホームの目的説明、チュートリアル、手入力で挑戦するゲーム任務1、送り方を選ぶゲーム任務2まで実装しています。旧ミッション1はチュートリアル、旧ミッション2はゲーム任務1として分離しました。詳細は[ゲーム仕様](docs/02_game_design.md)を参照してください。
...
5. **次：ゲーム任務3～4** — 修理と方式情報を追加。
6. **完了：圧縮の読みもの** — RLEを最初に学ぶ理由、技術の位置づけ、出典付きの歴史年表。ホーム・チュートリアル・ゲームで閲覧可能。
7. 自由実験と用語カードを追加。
8. RLE章（MVP）全体の受け入れ確認を行う。
9. **合意した後続の学習順：LZ77の考え方 → ハフマン → 組み合わせ → 実ファイルとZIP**。歴史上の発表順とは分け、「今の方式で送れないデータを、新しい工夫で送る」任務を作る。

直近の実装対象はRLE章の任務3「復元機を修理する」です。RLE章を一通り完成させた後、`ABCABCABC`の並びの繰り返しを題材に、LZ77の考え方へ進みます。後続章は未実装です。詳細は[学習ロードマップ](docs/05_acceptance_and_roadmap.md#学習ロードマップ合意済み)を参照してください。
...
任務ごとに下書き、方式、再生位置、ヒントの開示段階、クリア状況を保持します。ホーム経由で別の任務に移っても混ざりません。「任務を最初から」「もう一度挑戦」はその任務の空のRLE入力へ戻します。ページを再読み込みすると全任務の状態と進捗をリセットします。
...
この例は方式情報ありのコア実験です。チュートリアルとゲーム任務1・2の画面は方式を事前共有する設定なので、方式情報は0 Bです。
...
| [src/components/GameMission.tsx](src/components/GameMission.tsx) | ゲーム任務1・2の方式選択・編集・復元・結果 |
| [src/game/missions.ts](src/game/missions.ts) | チュートリアルとゲーム任務1・2の教材・予算・依頼・ヒント |
...
型検査・自動テスト104件（コア77件＋チュートリアル状態3件＋ゲーム状態24件）・ブラウザテスト24件・Webアプリとコアのビルドがすべて成功しました。
資料の境界値・異常系に加え、固定シードで生成した100入力の往復変換とトレースの一致を確認しています。旧5問の数値例、合法だが予算を超える組み方、欠損と修理もコアで検証しています。

- 操作できるのはホーム、チュートリアル、ゲーム任務1・2です。ゲーム任務3～4、自由実験、用語カードは未実装です。豆知識はRLEの方式名・仕組み・学ぶ理由・歴史と出典を掲載しています。
